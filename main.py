...
from forms import CreatePostForm, CreteNewUserForm, LoginUserForm, CommentForm
...
def login():
    form = LoginUserForm()
    if form.validate_on_submit():
        user_from_database = User.query.filter_by(email=form.email.data).first()
        if user_from_database:
            if check_password_hash(user_from_database.password, form.password.data):
...
def admin_only(function):
    ...
    def decorated_function():
        if current_user.is_authenticated and current_user.id == 1:
            ...
            flash('Voce precisa estar logado como Admin para acessar essa pagina.')
            return redirect(url_for('login'))
